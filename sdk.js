(function () {
  ...
  const SDK = {
    ...
    _initMock() {
      ...
      this._lang = nav.startsWith("ru") ? "ru" : "ru"; // forced RU for this project
      console.info("[SDK] Mock mode enabled.");
    },
    ...
    ready() {
      ...
      } catch (e) { console.warn("[SDK] ready() failed:", e); }
    },
    ...
    async cloudSave(data) {
      ...
      if (!this.ysdk || !this._player || typeof this._player.setData !== "function") {
        try { localStorage.setItem("mockCloudSave", JSON.stringify(data)); return true; } catch (_) { return false; }
      }
      try { await this._player.setData(data); return true; } catch (e) { console.warn("[SDK] cloudSave failed:", e); return false; }
    },

    async cloudLoad() {
      if (!this.isAuthenticated()) return null;
      if (!this.ysdk || !this._player || typeof this._player.getData !== "function") {
        try { const s = localStorage.getItem("mockCloudSave"); return s ? JSON.parse(s) : null; } catch (_) { return null; }
      }
      try { return (await this._player.getData()) || null; } catch (e) { console.warn("[SDK] cloudLoad failed:", e); return null; }
    },
